...
function findAnagrams(array){
    ...
    const checkAnagrams=(elem)=>{
        return elem.replace(/\s+/g, "").toLowerCase().split("").sort().join("")
        }
    
    return array.filter(([elem1,elem2])=>{
        return checkAnagrams(elem1)===checkAnagrams(elem2)
        })  
}
